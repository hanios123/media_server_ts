...
export class Room extends EventEmitter
{
	 ...
	 *   mediasoup Router video codecs.
	 */
	static async create({ mediasoupWorker , roomId, forceH264 = false, forceVP9 = false }:{ mediasoupWorker:any , roomId:string, forceH264:boolean, forceVP9:boolean })
	{
		logger.info(
	...
	constructor({ roomId, protooRoom, mediasoupRouter, audioLevelObserver, bot }: { roomId: string, protooRoom: any, mediasoupRouter: any, audioLevelObserver:any, bot:any })
	{
		super();
	 ...
	 */
	handleProtooConnection({ peerId, consume, protooWebSocketTransport }:{ peerId:any, consume:any, protooWebSocketTransport:any })
	{
	 ...
	 */
	async createBroadcaster({ id, displayName, device, rtpCapabilities }:{ id:String, displayName:String, device:any, rtpCapabilities:RTCRtpCapabilities })
	{
		...
		if (rtpCapabilities)
		{
			for (const joinedPeer of joinedPeers)
			{
				...
				for (const producer of joinedPeer.data.producers.values())
				{
					...
					(peerInfo.producers as any).push(
						{
							id   : producer.id ,
							kind : producer.kind
						});
	 ...
	 * @async
	 */
	async _createConsumer({ consumerPeer, producerPeer, producer }:{ consumerPeer:any, producerPeer:any, producer:any })
	{
		// Optimization:
		...
		try
		{
			consumer = await (transport as any).consume(
				{
					producerId      : producer.id,
					rtpCapabilities : consumerPeer.data.rtpCapabilities,
					paused          : true
	...
	async _createDataConsumer(
		{
			...
			dataProducerPeer:any, // This is null for the bot DataProducer.
			dataProducer:any
		})
	{
		...
		try
		{
			dataConsumer = await (transport as any).consumeData(
				{
					dataProducerId : dataProducer.id
				});
		}
